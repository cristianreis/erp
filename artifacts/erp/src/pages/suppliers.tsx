...
type SupplierForm = { name: string; document: string; email: string; phone: string; address: string; city: string; state: string; active: boolean; };
const defaultValues: SupplierForm = { name: "", document: "", email: "", phone: "", address: "", city: "", state: "", active: true };
...
export default function Suppliers() {
  ...
  const { data: rows = [], isLoading } = useListSuppliers({ search });
  const createM = useCreateSupplier({ mutation: { onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/suppliers"] }); toast({ title: "Fornecedor criado!" }); setDialogOpen(false); } } });
  const updateM = useUpdateSupplier({ mutation: { onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/suppliers"] }); toast({ title: "Atualizado!" }); setDialogOpen(false); } } });
  const deleteM = useDeleteSupplier({ mutation: { onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/suppliers"] }); toast({ title: "Excluído!" }); setDeleteRow(null); } } });
  ...
  const onSubmit = (data: SupplierForm) => editRow ? updateM.mutate({ id: editRow.id, data: data as any }) : createM.mutate({ data: data as any });
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader title="Fornecedores" description="Cadastro de fornecedores." onNew={openNew} newLabel="Novo Fornecedor" />
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{editRow ? "Editar Fornecedor" : "Novo Fornecedor"}</DialogTitle></DialogHeader>
          <Form {...form}>
      ...
      <AlertDialog open={!!deleteRow} onOpenChange={o => !o && setDeleteRow(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Excluir fornecedor?</AlertDialogTitle><AlertDialogDescription>Excluir <strong>{deleteRow?.name}</strong>?</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter>
