...
export default function PurchaseOrders() {
  ...
  const createM = useCreatePurchaseOrder({ mutation: { onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/purchase-orders"] }); toast({ title: "Pedido de compra criado!" }); setDialogOpen(false); } } });
  const form = useForm<PoForm>({ defaultValues: dfv });
  const { fields, append, remove } = useFieldArray({ control: form.control, name: "items" });
  const onSubmit = (data: PoForm) => {
    const items = data.items.map(i => ({ ...i, totalPrice: String(parseFloat(i.quantity) * parseFloat(i.unitPrice)) }));
  ...
  const columns = [
    ...
    { key: "expectedDate", header: "Previsão", render: (r: any) => r.expectedDate ? format(new Date(r.expectedDate), "dd/MM/yy", { locale: ptBR }) : "–" },
    { key: "status", header: "Status", render: (r: any) => <StatusBadge status={r.status} /> },
    { key: "totalAmount", header: "Total", className: "text-right", render: (r: any) => `R$ ${parseFloat(r.totalAmount ?? "0").toFixed(2)}` },
  ];

  return (
    <div className="flex flex-col gap-4">
      <PageHeader title="Pedidos de Compra" description="Gestão de pedidos de compra a fornecedores." onNew={() => { form.reset(dfv); setDialogOpen(true); }} newLabel="Novo Pedido" />
      <DataTable data={orders as any[]} columns={columns} isLoading={isLoading} emptyMessage="Nenhum pedido de compra encontrado." />
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="border rounded-md p-3 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" size="sm" variant="outline" onClick={() => append({ productId: "", quantity: "1", unitPrice: "" })}><Plus className="h-3 w-3 mr-1" /> Adicionar</Button>
                </div>
                {fields.map((field, i) => (
                  <div key={field.id} className="grid grid-cols-12 gap-2 items-end">
                    <div className="col-span-6"><FormField control={form.control} name={`items.${i}.productId`} rules={{ required: true }} render={({ field }) => (
                    ...
                    <div className="col-span-3"><FormField control={form.control} name={`items.${i}.unitPrice`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Preço Unit.</FormLabel><FormControl><Input type="number" step="0.01" className="text-xs" {...field} /></FormControl></FormItem>)} /></div>
